...
--Create departure dump table
CREATE OR REPLACE TABLE final.public.departure_dump
USING template (
    SELECT array_agg(object_construct(*))
    FROM TABLE(
        infer_schema(
          location=>'@project_stage/output_departure.parquet',
...
--Load data 
COPY INTO final.public.departure_dump
FROM @project_stage/output_departure.parquet
file_format = parquet_format
match_by_column_name = case_insensitive;

--Create arrival dump table 
CREATE OR REPLACE TABLE final.public.arrival
USING template (
    SELECT array_agg(object_construct(*))
    FROM TABLE(
        infer_schema(
          location=>'@project_stage/output_arrival.parquet',
          file_format=>'parquet_format'
        )
    )
);
...
--Load data
COPY INTO final.public.arrival
FROM @project_stage/output_arrival.parquet
file_format = parquet_format
match_by_column_name = case_insensitive;

ALTER TABLE final.public.departure_dump  
RENAME COLUMN "Departure_Time_UTC" TO departure_time_utc;

--Create refined departure table
CREATE OR REPLACE TABLE final.public.departure AS (
SELECT d.*,
       EXTRACT(year FROM departure_time_utc) AS year,
       EXTRACT(month FROM departure_time_utc) AS month,
       EXTRACT(day FROM departure_time_utc) AS day,
       EXTRACT(dayofweek FROM departure_time_utc) AS dow,
       DAYNAME(departure_time_utc) AS dayname,
       CASE WHEN EXTRACT(dayofweek FROM departure_time_utc) IN (0,6) THEN 1
            ELSE 0 END AS is_weekend,
       CASE WHEN EXTRACT(dayofweek FROM departure_time_utc) NOT IN (0,6) THEN 1
            ELSE 0 END AS is_weekday,
       CASE 
        WHEN EXTRACT(month FROM departure_time_utc) IN (3,4,5) THEN 'Spring'
        WHEN EXTRACT(month FROM departure_time_utc) IN (6,7,8) THEN 'Summer'
        WHEN EXTRACT(month FROM departure_time_utc) IN (9,10,11) THEN 'Autumn'
        WHEN EXTRACT(month FROM departure_time_utc) IN (12,1,2) THEN 'Winter' END AS season    
FROM final.public.departure_dump d
);
